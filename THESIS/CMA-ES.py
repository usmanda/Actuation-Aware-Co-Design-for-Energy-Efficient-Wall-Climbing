# CMA-ES → EXACT MPC WEIGHT OPTIMIZATION (TIME-BASED, 10s)
...
# =========================================================
# SINGLE CMA-ES EVALUATION (IDENTICAL TO PPO EPISODE)
# =========================================================
def run_episode(W, solver):
    ...
    if dz < MIN_HEIGHT_OK:
        return (E_act / dz) + LOW_HEIGHT_PENALTY * (MIN_HEIGHT_OK - dz)
...
if __name__ == "__main__":
    ...
    while not es.stop():
        solutions = es.ask()
        fitnesses = [objective(a) for a in solutions]
        es.tell(solutions, fitnesses)
        es.disp()

    best_a = es.result.xbest
    best_W = action_to_weights(best_a)
